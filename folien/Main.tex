...
\begin{itemize}
{\footnotesize
	\item[1)] wechselt in das Verzeichnis '/tmp'
	\item[2)] erstellt den Ordner 'test'
	\item[3)] guckt euch die Rechte des Ordners an
	\item[4)] entfernt die Recht für 'group' und 'all' des Ordners
	\item[5)] setzt als neuen Owner des Ordners 'root' [nur Root]
	\item[6)] versucht den Ordner zu entfernen (ohne 'sudo'). Was passiert?
	\item[7)] wechselt nun wieder den Owner zurück [nur Root]
	\item[8)] entfernt den Ordner
	}
\end{itemize}
...
\begin{itemize}
	{\footnotesize
	\item[1)] wechselt in euer Home-Verzeichnis
	\item[2)] lasst euch die Inhalte rekursiv ausgeben
	\item[3)] benutzt die Ausgabe als Eingabe für 'cowsay' (Pipe-Operator)
	\item[4)] lasst euch nun diese Ausgabe in die Datei 'KuhSagtHome.txt' schreiben
	\item[5)] seht euch den Inhalt der Datei an
	}
\end{itemize}
...
\begin{itemize}
	{\footnotesize
	\item[1)] sucht alle Dateien in '/usr/share' ausgeben, die dem folgenden RegEx entsprechen '.*.txt' und schreibt diese in '/home/[username]/txt-files.txt'
	\item[2)] schaut euch nun den Inhalt an
	\item[3)] sucht nach eurer Datei 'KuhSagtHome.txt' und schreibt das Ergebnis in 'kuh-sagt-path.txt' (Suche in '/home')
	\item[4)] verbindet nun den Inhalt von 'txt-files.txt' und 'kuh-sagt-path.txt' und schreibt die Ausgabe in 'cat-files-kuh.txt'
	\item[5)] betrachtet das Ende der Datei 'cat-files-kuh.txt'. Was fällt auf?
	\item[6)] sortiert den Inhalt von 'cat-files-kuh.txt' numerisch und schreibt die Ausgabe in 'sort-cat-kuh.txt'
	\item[7)] betrachtet den Anfang von 'sort-cat-kuh.txt'. Was hat sich verändert?
	\item[8)] löscht nun alle eben erstellten Dateien
	}
\end{itemize}
...
\begin{itemize}
	{\footnotesize
	\item[1)] ladet euch die Datei 'infinite.tar' von 'foss-ag.de' herunter
	\item[2)] verschiebt sie nach '/tmp' und entpackt sie dort
	\item[3)] führt den Befehl 'python infinite$\_$run.py' im Hintergrund aus mit Nice-Wert 15
	\item[4)] führt 'htop' bzw. 'top' aus, sucht euren Prozess und merkt euch die PID
	\item[5)] verändert den Nice-Wert auf 10 und schaut in 'htop' bzw. 'top' nach, was passiert ist
	\item[6)] stoppt nun den Prozess mit dem Befehl 'kill', wobei ihr die PID angeben müsst
	}
\end{itemize}
...
\begin{itemize}
{\footnotesize
	\item[1)] lasst euch eure IP-Adresse ausgeben und schreibt die Ausgabe in 'ip.txt'
	\item[2)] lasst euch nun nur die Zeilen Ausgeben, die eine valide IP-Adresse enthalten (IPv4, IPv6, oder beides)
